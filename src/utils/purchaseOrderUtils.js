...
export const generatePurchaseOrderPDF = async (purchaseId, settings, title = "Purchase Order") => {
  try {
    ...
    const drawSupplierSection = (y) => {
        ...
        const isAutoGenerated = finalDisp && (/^\d{12,}$/.test(String(finalDisp)) || /^20\d{12,}$/.test(String(finalDisp)));

        if (finalDisp && finalDisp !== "N/A" && !isAutoGenerated) {
            doc.text("PO No:", rightX + 2, headerY);
            doc.setFont("helvetica", "normal");
            doc.text(finalDisp, pageWidth - margin - 2, headerY, { align: "right" });
            headerY += 5;
        }
