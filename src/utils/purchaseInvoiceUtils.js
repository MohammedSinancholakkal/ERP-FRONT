...
export const generatePurchaseInvoicePDF = async (purchaseId, settings, title = "Purchase Invoice") => {
  try {
    ...
    const drawHeader = (y) => {
       ...
       const cGSTIN = settings?.gstin || settings?.GSTIN || "";
       if (cGSTIN) {
    ...
    const drawSupplierSection = (y) => {
        ...
        // If it's a timestamp (all digits, very long, or starts with YYYY), it was auto-generated by the system, so hide it.
        const isAutoGenerated = dbVNo && (/^\d{12,}$/.test(String(dbVNo)) || /^20\d{12,}$/.test(String(dbVNo)));

        if (dbVNo && !isAutoGenerated) {
            doc.text("Invoice No:", rightX + 2, headerY);
            doc.setFont("helvetica", "normal");
            doc.text(dbVNo, pageWidth - margin - 2, headerY, { align: "right" });
            headerY += 5;
        }

        doc.setFont("helvetica", "bold");
        doc.text("Date:", rightX + 2, headerY);
        doc.setFont("helvetica", "normal");
        doc.text(new Date(purchase.Date || purchase.date).toLocaleDateString(), pageWidth - margin - 2, headerY, { align: "right" });
