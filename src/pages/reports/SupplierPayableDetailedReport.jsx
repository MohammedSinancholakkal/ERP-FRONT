...
import { getSupplierPayablesDetailedApi } from "../../services/allAPI";

const SupplierPayableDetailedReport = () => {
  ...
  const fetchReport = async () => {
      try {
          setLoading(true);
          const res = await getSupplierPayablesDetailedApi(sortConfig.key, sortConfig.direction);
          if (res.status === 200) {
              setData(res.data);
          } else {
        ...
        <ContentCard>
          <div className="flex flex-col h-full overflow-hidden gap-4">
            ...
            <div className="flex justify-between items-center">
              <h2 className={`text-xl font-bold ${theme === 'purple' ? 'text-purple-800' : theme === 'emerald' ? 'text-emerald-800' : 'text-white'}`}>Supplier Payable Detailed Report</h2>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead className={`sticky top-0 z-10 ${theme === 'purple' ? 'bg-purple-100 text-purple-900' : theme === 'emerald' ? 'bg-emerald-100 text-emerald-900' : 'bg-gray-800 text-gray-300'}`}>
                  <tr>
                    ...
                    <th className="px-4 py-3 cursor-pointer select-none font-medium hover:bg-black/5" onClick={() => handleSort('companyName')}>
                      Supplier Name {sortConfig.key === 'companyName' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                    </th>
                ...
                <tbody className={`divide-y ${theme === 'purple' ? 'divide-purple-100' : theme === 'emerald' ? 'divide-emerald-100' : 'divide-gray-700'}`}>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="5" className={`text-center py-8 ${theme === 'purple' ? 'text-purple-600' : theme === 'emerald' ? 'text-emerald-600' : 'text-gray-400'}`}>No suppliers found.</td>
                    </tr>
                  ) : (
                    paginatedData.map((supplier) => (
                      <React.Fragment key={supplier.id}>
                        <tr className={`${theme === 'purple' ? 'hover:bg-purple-50 bg-white text-gray-900' : theme === 'emerald' ? 'hover:bg-emerald-50 bg-white text-gray-900' : 'hover:bg-gray-700 bg-gray-800 text-gray-200'} cursor-pointer transition-colors duration-200`} onClick={() => toggleRow(supplier.id)}>
                          ...
                          <td className="px-4 py-3 font-semibold">{supplier.companyName}</td>
                          <td className="px-4 py-3 text-right">{(supplier.totalPayable || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          <td className="px-4 py-3 text-right">{(supplier.totalPaid || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          <td className="px-4 py-3 text-right font-bold">{(supplier.balance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        </tr>
                        
                        {/* Expanded Child Rows */}
                        {expandedRows[supplier.id] && supplier.transactions && supplier.transactions.length > 0 && (
                          <tr>
                                    ...
                                    <table className={`w-full text-sm text-left border rounded-lg overflow-hidden shadow-inner ${theme === 'emerald' || theme === 'purple' ? 'border-gray-200 bg-white' : 'border-gray-700 bg-gray-800'}`}>
                                        <thead className={`${theme === 'emerald' || theme === 'purple' ? 'bg-gray-100 text-gray-700' : 'bg-gray-900 text-gray-300'}`}>
                                            <tr>
                                                ...
                                                <th className="px-3 py-2 text-right font-medium">Purchase Amount</th>
                                                <th className="px-3 py-2 text-right font-medium">Paid Amount</th>
                                            </tr>
                                        </thead>
                                                ...
                                                <tr key={txn.id} className={`${theme === 'emerald' || theme === 'purple' ? 'hover:bg-gray-50 text-gray-800' : 'hover:bg-gray-700 text-gray-200'} transition-colors`}>
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                            ${(txn.type || '').toLowerCase().includes('payment') || txn.paid > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                            {txn.type || 'N/A'}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-2 text-right text-red-600 font-medium">{txn.amount > 0 ? txn.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                                                    <td className="px-3 py-2 text-right text-green-600 font-medium">{txn.paid > 0 ? txn.paid.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
...
export default SupplierPayableDetailedReport;
