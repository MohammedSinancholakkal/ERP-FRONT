...
import { getCashInHandReportApi } from "../../services/allAPI";
...
const CashInHandReport = () => {
  ...
  const fetchReport = async () => {
      try {
          setLoading(true);
          const res = await getCashInHandReportApi(page, limit, sortConfig.key, sortConfig.direction);
          if (res.status === 200) {
        ...
        <ContentCard>
          <div className="flex flex-col h-full overflow-hidden gap-4">
            ...
            <div className="flex justify-between items-center">
              <h2 className={`text-xl font-bold ${theme === 'purple' ? 'text-purple-800' : theme === 'emerald' ? 'text-emerald-800' : 'text-white'}`}>Cash In Hand Report (Purchases & Sales)</h2>
                        ...
                        <tr key={record.transactionId} className={`${theme === 'purple' ? 'bg-white hover:bg-purple-50 text-gray-900' : theme === 'emerald' ? 'bg-white hover:bg-emerald-50 text-gray-900' : 'bg-gray-800 hover:bg-gray-700 text-gray-200'} transition-colors duration-200`}>
                          ...
                          <td className="px-4 py-3">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                  ${(record.type === 'INV' || record.type === 'RECEIPT') ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'}`}>
                                  {record.type === 'INV' ? 'Sale' : record.type === 'PURCHASE' ? 'Purchase' : record.type === 'RECEIPT' ? 'Sale Receipt' : record.type === 'PAYMENT' ? 'Purchase Payment' : record.type}
                              </span>
                          </td>
