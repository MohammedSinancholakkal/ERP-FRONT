...
const CreditVoucher = () => {
  ...
  const [searchDebitHead, setSearchDebitHead] = useState("");
  const [debitHeadOpen, setDebitHeadOpen] = useState(false);
  ...
  const [rows, setRows] = useState(sampleCreditV);
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative w-[750px] bg-gradient-to-b from-gray-900 to-gray-800 border border-gray-700 rounded-lg text-white p-5 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-gray-700 pb-3">
              <h2 className="text-lg font-semibold">Add Credit Voucher</h2>
              <X
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setSearchDebitHead(e.target.value);
                      setDebitHeadOpen(true);
                    }}
                    onClick={() => setDebitHeadOpen(true)}
                    placeholder="Search account..."
                    className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2"
                  />

                  {debitHeadOpen && (
                    <div className="absolute left-0 right-0 bg-gray-800 border border-gray-700 rounded mt-1 max-h-40 overflow-auto z-50">
                      {debitAccountHeadOptions
                        .filter((v) =>
                          v.toLowerCase().includes(searchDebitHead.toLowerCase())
                        )
                        .map((v) => (
                          <div
                            key={v}
                            onClick={() => {
                              setForm((p) => ({ ...p, debitAccountHead: v }));
                              setDebitHeadOpen(false);
                            }}
...
<div className="p-4 text-white bg-gradient-to-b from-gray-900 to-gray-700 h-full">
  <div className="flex flex-col h-full overflow-hidden"> 
        <h2 className="text-2xl font-semibold mb-4">Credit Voucher</h2>
            ...
            <table className="min-w-[1000px] border-separate border-spacing-y-1 text-sm table-fixed">
              <thead className="sticky top-0 bg-gray-900 z-10">
                <tr className="text-center">
                  ...
                  {visibleColumns.debitAccountHead && (
                    <th className="pb-2 border-b">Debit A/C Head</th>
                  )}
                  {visibleColumns.account && (
                    <th className="pb-2 border-b">Account</th>
                  )}
              ...
              <tbody className="text-center">
                {rows.map((r) => (
                  ...
                  >
                    ...
                    {visibleColumns.vType && <td className="py-2">Credit Voucher</td>}
                    {visibleColumns.date && <td className="py-2">{r.date}</td>}
                    {visibleColumns.debitAccountHead && (
                      <td className="py-2">{r.debitAccountHead}</td>
                    )}
                    {visibleColumns.account && (
