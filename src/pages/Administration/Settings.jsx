...
const Settings = () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (data && data.id) {
          ...
          if (data.invoiceLogoPath) setInvoiceLogo(`${baseUrl}/${data.invoiceLogoPath}`);
  ...
  const validateForm = () => {
    ...
    if (!String(vatPercent).trim()) newErrors.vatPercent = "VAT Percentage is required";
    if (!vatType.trim()) newErrors.vatType = "VAT Type is required";
    if (!currencyPosition.trim()) newErrors.currencyPosition = "Currency Position is required";
  ...
  const handleSave = async () => {
    ...
    // TODO: Add userId from auth context or similar
    formData.append("userId", 1); // Hardcoded for now as per previous code context

   if (!logoFile && logo && !logo.startsWith("blob:")) {
     const baseUrl = serverURL.replace("/api", "");
     formData.append("logoPath", logo.replace(`${baseUrl}/`, ""));
}

if (!invoiceLogoFile && invoiceLogo && !invoiceLogo.startsWith("blob:")) {
  const baseUrl = serverURL.replace("/api", "");
  formData.append("invoiceLogoPath", invoiceLogo.replace(`${baseUrl}/`, ""));
}

if (!faviconFile && favicon && !favicon.startsWith("blob:")) {
  const baseUrl = serverURL.replace("/api", "");
  formData.append("faviconPath", favicon.replace(`${baseUrl}/`, ""));
}
          ...
          <div className="max-w-[1500px]">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {currencies.map((c) => (
                  <option key={c.id} value={c.currencyName}>{c.currencyName} ({c.currencySymbol})</option>
                ))}
              </select>
            ...
            {errors.currency && (
              <p className="text-red-400 text-sm mt-1 mb-6">{errors.currency}</p>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div>
                ...
                />
                {errors.vatPercent && (
                  <p className="text-red-400 text-sm mt-1">{errors.vatPercent}</p>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="w-full h-40 bg-gray-900 border border-gray-700 rounded flex items-center justify-center relative">
                  {!logo ? (
                    <label className="cursor-pointer px-4 py-2 bg-gray-800 border border-gray-600 rounded">
                      <input type="file" className="hidden" onChange={(e) => handleImageSelect(e, setLogo, setLogoFile)} />
                      Select File
                    </label>
                  ) : (
                    <div className="relative">
                      <img src={logo} className="h-36 rounded border border-gray-700" />
                      <button
              ...
              <div>
                ...
                <div className="w-full h-40 bg-gray-900 border border-gray-700 rounded flex items-center justify-center relative">
                  {!invoiceLogo ? (
                    <label className="cursor-pointer px-4 py-2 bg-gray-800 border border-gray-600 rounded">
                      <input type="file" className="hidden" onChange={(e) => handleImageSelect(e, setInvoiceLogo, setInvoiceLogoFile)} />
                      Select File
                    </label>
                  ) : (
                    <div className="relative">
                      <img src={invoiceLogo} className="h-36 rounded border border-gray-700" />
                      <button
                        className="absolute top-1 right-1 bg-red-600 p-1 rounded-full"
                        onClick={() => removeImage(setInvoiceLogo, setInvoiceLogoFile)}
                      >
                        <X size={14} />
                      </button>
                    </div>
              ...
              <div>
                ...
                <div className="w-full h-40 bg-gray-900 border border-gray-700 rounded flex items-center justify-center relative">
                  {!favicon ? (
                    <label className="cursor-pointer px-4 py-2 bg-gray-800 border border-gray-600 rounded">
                      <input type="file" className="hidden" onChange={(e) => handleImageSelect(e, setFavicon, setFaviconFile)} />
                      Select File
                    </label>
                  ) : (
                    <div className="relative">
                      <img src={favicon} className="h-36 rounded border border-gray-700" />
                      <button
                        className="absolute top-1 right-1 bg-red-600 p-1 rounded-full"
            ...
            {errors.currencyPosition && (
              <p className="text-red-400 text-sm mt-1">{errors.currencyPosition}</p>
            )}
