                ...
                <thead className="sticky top-0 bg-gray-900">
                  <tr>
                    ...
                    {visibleColumns.details && (
                      <th className="pb-1 border-b text-center">Actions</th>
                    )}
                  </tr>
                </thead>
                ...
                <tbody>
                  {salesData.map((s) => (
                    ...
                    >
                      ...
                      {visibleColumns.date && <td className="px-2 py-2">{s.date}</td>}
                      {visibleColumns.paymentAccount && (
                        <td className="px-2 py-2">{s.paymentAccount}</td>
                      )}
                      {visibleColumns.discount && (
                        <td className="px-2 py-2">{s.discount}</td>
                      )}
                      {visibleColumns.totalDiscount && (
                        <td className="px-2 py-2">{s.totalDiscount}</td>
                      )}
                      {visibleColumns.vat && <td className="px-2 py-2">{s.vat}</td>}
                      {visibleColumns.totalTax && (
                        <td className="px-2 py-2">{s.totalTax}</td>
                      )}
                      {visibleColumns.shippingCost && (
                        <td className="px-2 py-2">{s.shippingCost}</td>
                      )}
                      {visibleColumns.grandTotal && (
                        <td className="px-2 py-2">{s.grandTotal}</td>
                      )}
                      {visibleColumns.netTotal && (
                        <td className="px-2 py-2">{s.netTotal}</td>
                      )}
                      {visibleColumns.paidAmount && (
                        <td className="px-2 py-2">{s.paidAmount}</td>
                      )}
                      {visibleColumns.due && <td className="px-2 py-2">{s.due}</td>}
                      {visibleColumns.change && (
                        <td className="px-2 py-2">{s.change}</td>
                      )}

                      {visibleColumns.details && (
                        <td className="px-2 py-2 text-center">{s.details}</td>
                      )}
                    </tr>
                  ))}
                </tbody>
