...
const Deductions = () => {
  ...
  const [inactiveDeductions, setInactiveDeductions] = useState([]);
  ...
  const loadInactive = async () => {
    try {
      ...
      if (res?.status === 200) {
        ...
        setInactiveDeductions(normalized);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSearch = async (text) => {
    ...
    if (!text.trim()) {
        setPage(1);
        return loadDeductions();
    }
    try {
      const res = await searchDeductionApi(text);
      if (res?.status === 200) {
        const rows = res.data || [];
        const normalized = rows.map(r => ({
            id: r.Id || r.id,
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await addDeductionApi({ ...newData, userId });
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await updateDeductionApi(editData.id, {
        name: editData.name,
  ...
  const handleDelete = async () => {
    try {
      const res = await deleteDeductionApi(editData.id, { userId });
      if (res?.status === 200) {
  ...
  const handleRestore = async () => {
    try {
      const res = await restoreDeductionApi(editData.id, { userId });
      if (res?.status === 200) {
      ...
      <div className="p-4 text-white bg-gradient-to-b from-gray-900 to-gray-700 h-full">
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
             <div className="flex items-center bg-gray-700 px-3 py-1.5 rounded border border-gray-600 w-full sm:w-60">
              ...
              {hasPermission(PERMISSIONS.DEDUCTIONS.CREATE) && (
          ...
          <div className="flex-grow overflow-auto min-h-0">
            <table className="w-[600px] border-separate border-spacing-y-1 text-sm">
                ...
                <tbody>
                    ...
                    {showInactive && inactiveDeductions.map(r => (
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
            <div className="w-[700px] bg-gray-900 text-white rounded-lg border border-gray-700">
               <div className="flex justify-between px-5 py-3 border-b border-gray-700">
                  <h2 className="font-semibold">New Deduction</h2>
       ...
       {editModalOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
            <div className="w-[700px] bg-gray-900 text-white rounded-lg border border-gray-700">
               <div className="flex justify-between px-5 py-3 border-b border-gray-700">
                  <h2 className="font-semibold">{editData.isInactive ? "Restore Deduction" : "Edit Deduction"}</h2>
