...
const AgendaItemTypes = () => {
  ...
  // Column picker (default: id + name)
  const defaultColumns = { id: true, name: true };
  const [visibleColumns, setVisibleColumns] = useState(defaultColumns);

  // For modal column picker (temp copy, apply on OK)
...
if (sortOrder === "asc") {
  ...
  const loadRows = async () => {
    try {
      ...
      if (res?.status === 200) {
        ...
        // Normalize: map DB fields to ui fields (id, name)
        const normalized = items.map((r) => ({
          id: r.Id ?? r.id ?? r.agendaItemTypeId ?? null,
          name: r.Name ?? r.AgendaTypeName ?? r.name ?? r.agendaItemTypeName ?? "",
        }));
      ...
      console.error("Load agenda item types error:", err);
      toast.error("Failed to load agenda item types");
    }
  };
  ...
  const openColumnPicker = () => {
    setTempVisibleColumns(visibleColumns); // copy current
  ...
  const cancelColumnPicker = () => {
    setTempVisibleColumns(visibleColumns); // discard changes
    setColumnModalOpen(false);
  };
    ...
    <>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="w-[520px] bg-gradient-to-b from-gray-900 to-gray-800 text-white rounded-lg border border-gray-700">
            ...
            <div className="p-6">
              <label className="text-sm text-gray-300">Name <span className="text-red-400">*</span></label>
              <input
                type="text"
                value={newItem.name}
            ...
            <div className="px-5 py-3 border-t border-gray-700 flex justify-end gap-2">
              <button
                onClick={() => { setModalOpen(false); setNewItem({ name: "" }); }}
      ...
      {editModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="w-[520px] bg-gradient-to-b from-gray-900 to-gray-800 text-white rounded-lg border border-gray-700">
            <div className="flex justify-between px-5 py-3 border-b border-gray-700">
              <h2 className="text-lg font-semibold">Edit Agenda Item Type</h2>
      ...
      {/* COLUMN PICKER (temp changes + OK/Cancel) */}
{/* ================= COLUMN PICKER ================= */}
{columnModalOpen && (
    ...
    <div className="w-[700px] bg-gradient-to-b from-gray-900 to-gray-800 border border-gray-700 rounded-lg text-white">
      ...
      <div className="px-5 py-3 border-t border-gray-700 flex justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => {
              setVisibleColumns(tempVisibleColumns); // apply changes
              setColumnModalOpen(false);
            }}
            className="px-3 py-2 bg-gray-800 border border-gray-600 rounded"
          >
      ...
      <div className="p-4 sm:p-6 text-white min-h-[calc(100vh-64px)] bg-gradient-to-b from-gray-900 to-gray-700 flex flex-col">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <button
            onClick={() => { setSearchText(""); setPage(1); loadRows(); }}
    ...
    <tbody className="text-center">
      ...
      {sortedRows.map((row) => (
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3 bg-gray-900/50 px-4 py-2 border border-gray-700 rounded text-sm">
          ...
          <span>Showing <b>{Math.min(totalRecords === 0 ? 0 : start, totalRecords)}</b> to <b>{end}</b> of <b>{totalRecords}</b> records</span>
        </div>
      </div>
    </>
  );
