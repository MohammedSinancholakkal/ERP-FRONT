...
const AttendeeTypes = () => {
  ...
  const { 
    refreshAttendeeTypes: refreshCtx, 
    refreshInactiveAttendeeTypes: refreshInactiveCtx 
  } = useMasters();
  // ===============================
  // State Declarations
  // ===============================
  ...
  const normalizeRows = (items = []) =>
    items.map((r) => ({
      id: r.Id ?? r.id ?? r.attendeeTypeId ?? null,
      name: capitalize(r.Name ?? r.AttendeeTypeName ?? r.name ?? ""),
    }));
  ...
  const loadRows = async () => {
    try {
      const res = await getAttendeeTypesApi(page, limit, sortConfig.key, sortConfig.direction);
      if (res?.status === 200) {
        const data = res.data;
        let items = [];
        ...
        toast.error("Failed to load attendee types");
      }
    } catch (err) {
      console.error(err);
      toast.error("Failed to load attendee types");
  ...
  const handleAdd = async () => {
    ...
    try {
      const searchRes = await searchAttendeeTypeApi(newItem.name.trim());
      if (searchRes?.status === 200) {
        ...
        if (existing) return toast.error("Attendee type with this name already exists");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleUpdate = async () => {
    ...
    if (!/^[a-zA-Z\s]+$/.test(nameToCheck)) return toast.error("Name allows only characters");
     ...
     // Check for duplicates
     try {
      const searchRes = await searchAttendeeTypeApi(editItem.name.trim());
      if (searchRes?.status === 200) {
        const rows = Array.isArray(searchRes.data)
          ? searchRes.data
          : searchRes.data?.records || [];
  ...
  return (
    ...
    <div className={`p-6 h-full ${theme === 'emerald' ? 'bg-gradient-to-br from-emerald-100 to-white text-gray-900' : theme === 'purple' ? 'bg-gradient-to-br from-gray-50 to-gray-200 text-gray-900' : 'bg-gradient-to-b from-gray-900 to-gray-700 text-white'}`}>
      ...
      <div className="flex flex-col h-full overflow-hidden gap-2">
        ...
        <h2 className={`text-xl font-bold ${theme === 'purple' ? 'text-purple-800' : theme === 'emerald' ? 'text-emerald-800' : 'text-white'}`}>Attendee Types</h2>
        ...
        <MasterTable
            ...
            permissionCreate={hasPermission(PERMISSIONS.ATTENDEE_TYPES.CREATE)}
            onRefresh={() => {
                const willUpdate = 
                    page !== 1 || 
                    limit !== 25 || 
