...
const Incomes = () => {
  ...
  const [inactiveIncomes, setInactiveIncomes] = useState([]);
  ...
  const handleSearch = async (text) => {
    ...
    try {
      const res = await searchIncomeApi(text);
      if (res?.status === 200) {
        const rows = res.data || [];
        const normalized = rows.map(r => ({
            id: r.Id || r.id,
  ...
  const handleDelete = async () => {
    try {
      const res = await deleteIncomeApi(editData.id, { userId });
      if (res?.status === 200) {
  ...
  const handleRestore = async () => {
    try {
      const res = await restoreIncomeApi(editData.id, { userId });
      if (res?.status === 200) {
          ...
          <div className="flex-grow overflow-auto min-h-0">
            <table className="w-[600px] border-separate border-spacing-y-1 text-sm">
                ...
                <tbody>
                    ...
                    {showInactive && inactiveIncomes.map(r => (
